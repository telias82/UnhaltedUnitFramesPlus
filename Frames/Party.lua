...
-- Single PLAYER_TARGET_CHANGED handler covers both party and raid frames.
local function sweep(header, dbKey)
    if not header or not UUFPLUS.db then return end
    local tDb = UUFPLUS.db.profile[dbKey].Indicators.Target
    local children = {header:GetChildren()}
    for i = 1, #children do
        local child = children[i]
        if child and child.unit and child._targetGlow then
            if tDb and tDb.Enabled and UnitIsUnit(child.unit, "target") then
...
local function UpdateAllTargetGlows()
    sweep(UUFPLUS.PartyHeader, "Party")
    sweep(UUFPLUS.RaidHeader,  "Raid")
end
...
do
    ...
    evtFrame:SetScript("OnEvent", UpdateAllTargetGlows)
end
...
function UUFPLUS:SpawnPartyFrames()
    ...
    do
        ...
        posFrame:SetScript("OnEvent", function()
            if not _partyPosPending then
                _partyPosPending = true
                C_Timer.After(0, function()
                    _partyPosPending = false
                    enforcePartyPos()
                end)
            end
        end)
    ...
    do
        local function sweepPartyRoleIcons()
            local h = UUFPLUS.PartyHeader
            if not h then return end
            local children = {h:GetChildren()}
            for i = 1, #children do
        ...
        combatFrame:SetScript("OnEvent", sweepPartyRoleIcons)
    end
end
